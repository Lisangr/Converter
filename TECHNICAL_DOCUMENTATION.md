...
Основные блоки:
- Инициализация UI динамически в `BuildUi()` и подметодах: `BuildLeftPanel`, `BuildRightPanel`, `BuildBottomPanel`, `BuildVideoTab` (включает настройки вывода), `BuildAudioTab`, `BuildAdvancedTab` (скрыта, значения задаются по умолчанию).
...
- Вкладки настроек:
  - Видео: формат, видеокодек, качество (CRF), масштаб (пресет или процент), разрешение-пресет, а также настройки вывода (папка, подпапка `Converted`, шаблон имени `{original}`, `{format}`, `{codec}`, `{resolution}`).
  - Аудио: включение/выключение, кодек, битрейт.
  - Дополнительно: скрыто; путь к FFmpeg, `-threads`, аппаратное ускорение — инициализируются значениями по умолчанию.
- Нижняя панель: прогресс общий/текущий, кнопки Старт/Стоп, сохранение/загрузка пресетов, журнал. Между прогрессом и журналом — регулируемая вертикальная граница (SplitContainer).
...
2. Для каждого файла:
   ...
   - Формирование фильтра `scale` (только при перекодировании видео):
...
3. Построение конверсии через Xabe:
   - Общие параметры: `-loglevel verbose`, `-i <input>`.
   - Видео:
     - Режим копирования: `-c:v copy` (без `-vf`, `-pix_fmt`, `-crf`).
     - Перекодирование: `-c:v <vcodec>`, опционально `-vf <scale>`, `-pix_fmt yuv420p`.
     - Для H.264/H.265: `-crf <N> -preset medium`.
   - Потоки: `-threads <N>` если > 0.
   - Аудио:
     - Режим копирования: `-c:a copy`.
     - Перекодирование: `-c:a <acodec> -b:a <abitrate>` или `-an`.
   - Особые случаи: для GIF аудио отключается (`-an`).
