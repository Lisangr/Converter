...
- [x] Добавить корректную деактивацию в Host.StopAsync
  - Реализовано корректное завершение всех сервисов при остановке Host

### 1.3 ServiceProvider lifecycle
**Цель**: Автоматическое освобождение контейнера

**Изменения**:
- [x] Использовать `using` для ServiceProvider в Program.cs
  - Оставлен только один ServiceProvider в Host
- [x] Перенести в Host для автоматического управления
  - Реализовано через HostBuilder
  - Все сервисы регистрируются и освобождаются корректно
...
- [x] Убрать конфликты между кэшем и хранилищем
  ...
  - Удалены устаревшие методы и дублирующая логика
...
## СЛЕДУЮЩИЕ ШАГИ:
1. **ЭТАП 2** (Поток данных) - удалить старый UI (filesPanel)
   - [ ] Удалить неиспользуемые файлы и ресурсы
   - [ ] Обновить документацию
2. **ЭТАП 6** (IDisposable) - завершить настройку освобождения ресурсов
   - [ ] Проверить освобождение всех управляемых ресурсов
   - [ ] Протестировать утечки памяти
3. **ЭТАП 8** - провести полное тестирование
